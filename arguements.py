...
DIVIDE_X = 10  # 划分图形区域的最小X值
DIVIDE_Y = 10  # 划分图形区域的最小Y值
MIN_POINTS_LIMIT = 20  # 有效图形区域的最小坐标点数量限制，小于该值视为无效区域
MS_PER_PIX = 1.4  # 跳跃1px距离需要长按约1.4ms(粗率计算)
...
MIN_GRAYSCALE_LIMIT = 5  # 最小灰度限制，用于过滤掉图形边界以外的点
TIME_INTERVAL = 1.8  # 两次跳跃的间隔时间(秒)，最好大于1.5，避免模型落地前就开始下一次识别

ADB_PATH = "～/Apps/adb/platform-tools/adb"  # ADB调用路径

# DEBUG调试参数
DEBUG_MODE = False  # 是否开启DEBUG模式，开启的话将保存原截图及处理后图片
PIC_DIR = "./pic"  # 图片保存路径
PIC_NAME = '1514787541.png'  # 使用None值，则通过adb获取截图; 指定图片名称，则从PIC_DIR下查找图片文件(仅DEBUG模式)
