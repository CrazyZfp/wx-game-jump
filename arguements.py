# !!! 下列是根据1920*1080分辨率预设参数,请根据实际分辨率调整 !!!
# !!! 下列是根据1920*1080分辨率预设参数,请根据实际分辨率调整 !!!
# !!! 下列是根据1920*1080分辨率预设参数,请根据实际分辨率调整 !!!
...
MS_PER_PIX = 1.36  # 跳跃1px距离需要长按约1.36ms(粗率计算)
...
MIN_RGB_TOLERANCE = 10  # 最小R/G/B容差,用于图形边界识别
TIME_INTERVAL = 1.6  # 两次跳跃的间隔时间(秒),推荐不小于1.5,避免模型落地前就开始下一次识别

ADB_PATH = "/home/zfp/Apps/adb/platform-tools/adb"  # ADB调用路径

# DEBUG调试参数
DEBUG_MODE = False  # 是否开启DEBUG模式,开启的话将保存原始截图及处理后图片
PIC_DIR = "./pic"  # 图片保存路径
PIC_NAME = None  # 使用None值,则通过adb获取截图; 指定图片名称,则从PIC_DIR下查找图片文件(仅DEBUG模式有效)
